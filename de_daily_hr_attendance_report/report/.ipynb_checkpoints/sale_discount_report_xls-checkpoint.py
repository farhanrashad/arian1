...
class ProductXlsx(models.AbstractModel):
    ...
    def _get_pos_sale(self, line_ids):
        query = """ SELECT
            ...
            CASE WHEN (pol.discount + pol.promo_disc_percentage) >= 0 and (pol.discount + pol.promo_disc_percentage) <= 10 THEN sum(pol.price_subtotal_incl) else 0.0 END as upto10,
            CASE WHEN (pol.discount + pol.promo_disc_percentage) > 10 and (pol.discount + pol.promo_disc_percentage) <= 20 THEN sum(pol.price_subtotal_incl) else 0.0 END as upto20,
            CASE WHEN (pol.discount + pol.promo_disc_percentage) > 20 and (pol.discount + pol.promo_disc_percentage) <= 30 THEN sum(pol.price_subtotal_incl) else 0.0 END as upto30,
            CASE WHEN (pol.discount + pol.promo_disc_percentage) > 30 and (pol.discount + pol.promo_disc_percentage) <= 40 THEN sum(pol.price_subtotal_incl) else 0.0 END as upto40,
            CASE WHEN (pol.discount + pol.promo_disc_percentage) > 40 and (pol.discount + pol.promo_disc_percentage) <= 50 THEN sum(pol.price_subtotal_incl) else 0.0 END as upto50,
            CASE WHEN (pol.discount + pol.promo_disc_percentage) > 50 and (pol.discount + pol.promo_disc_percentage) <= 60 THEN sum(pol.price_subtotal_incl) else 0.0 END as upto60,
            CASE WHEN (pol.discount + pol.promo_disc_percentage) > 60 and (pol.discount + pol.promo_disc_percentage) <= 70 THEN sum(pol.price_subtotal_incl) else 0.0 END as upto70,
            CASE WHEN (pol.discount + pol.promo_disc_percentage) > 70 and (pol.discount + pol.promo_disc_percentage) <= 80 THEN sum(pol.price_subtotal_incl) else 0.0 END as upto80,
            CASE WHEN (pol.discount + pol.promo_disc_percentage) > 80 and (pol.discount + pol.promo_disc_percentage) <= 90 THEN sum(pol.price_subtotal_incl) else 0.0 END as upto90,
            CASE WHEN (pol.discount + pol.promo_disc_percentage) > 90 and (pol.discount + pol.promo_disc_percentage) <= 100 THEN sum(pol.price_subtotal_incl) else 0.0 END as upto100

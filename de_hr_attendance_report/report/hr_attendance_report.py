    ...
    def _get_report_values(self, docids, data=None):
        ...
        for employee in docs.employee_ids:
            ...
            emp_attendance = self.env['hr.attendance'].search(
                [('employee_id', '=', employee1.id), ('check_in', '>=', date_from), ('check_out', '<=', date_to),
                 ('company_id', '=', employee1.company_id.id)])
            ...
            emp_leaves = self.env['hr.leave'].search(
                [('employee_id', '=', employee1.id), ('date_from', '>=', date_from), ('date_to', '<=', date_to),
                 ('state', '=', 'validate')])
            previous_date = fields.date.today()
            ...
            for timeoff_type in uniq_leave_type:
                leave_work_days = 0
                leaves_work_hours = 0
                emp_leaves_type = self.env['hr.leave'].search(
                    [('holiday_status_id', '=', timeoff_type), ('employee_id', '=', employee1.id),
                     ('date_from', '>=', date_from), ('date_to', '<=', date_to), ('state', '=', 'validate')])
                ...
                timeoff_work_entry_type = self.env['hr.work.entry.type'].search([('code', '=', timeoff_vals.name)],
                                                                                limit=1)
            ...
            rest_days_initial = 0
            absent_work_days = 0
            shift_contract_lines = self.env['hr.shift.schedule.line'].search(
                [('employee_id', '=', employee1.id), ('date', '>=', date_from), ('date', '<=', date_to),
                 ('state', '=', 'posted')])
            ...
            if not absent_work_entry_type:
                ...
                work_entry = self.env['hr.work.entry.type'].create(vals)
            apply_leave_days = 0
            emp_leaves_apply = self.env['hr.leave'].search(
                [('employee_id', '=', employee1.id), ('date_from', '>=', date_from), ('date_to', '<=', date_to),
                 ('state', '=', 'validate')])
            ...
            for i in range(0, total_days + 1):
                ...
                hr_attendance = self.env['hr.attendance'].search(
                    [('employee_id', '=', employee.id), ('attendance_date', '=', date_after_month)])
                if hr_attendance:
                    for attendance in hr_attendance:

                        emp_leaves = self.env['hr.leave'].search(
                            [('employee_id', '=', employee.id), ('request_date_from', '=', date_after_month)], limit=1)
                        ...
                        if not shift:
                            ...
                            shift = self.env['resource.calendar'].search([('company_id', '=', employee.company_id.id)],
                                                                         limit=1).name
                            ...
                            shift_line = self.env['hr.shift.schedule.line'].search(
                                [('employee_id', '=', employee.id), ('date', '=', date_after_month)], limit=1)
                        ...
                        rest_schedule_line = self.env['hr.shift.schedule.line'].search(
                            [('employee_id', '=', employee.id), ('date', '=', date_after_month),
                             ('rest_day', '=', True)], limit=1)
                        ...
                        for gazetted_day in current_shift.global_leave_ids:
                            if str(request_date) >= str(gazetted_day.date_from) and str(request_date) <= str(
                                    gazetted_day.date_to):
                    ...
                    rest_schedule_line = self.env['hr.shift.schedule.line'].search(
                        [('employee_id', '=', employee.id), ('date', '=', date_after_month), ('rest_day', '=', True)],
                        limit=1)

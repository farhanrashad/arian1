...
class ExtraIssuanceReport(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
        ...
        bom_product = []
        all_boms = []
        if docs.articles_lines:
            for sale in docs.articles_lines: 
                article_qty = sale.quantity
                if docs.is_quantity == True:
                    article_qty = sale.product_quantity
                ...
                if  article_qty > 0.0:   
                    ...
                    if product_variant_bom:
                        if product_variant_bom.type == 'normal':
                            bom_vals = {
        ...
        elif docs.articles_lines2:
            for sale in docs.articles_lines2: 
                article_qty = sale.quantity
                if docs.is_quantity == True:
                    article_qty = sale.product_quantity
        ...
        for uniq in uniq_product:
            product_exist = self.env['product.product'].search([('id','=',uniq)], limit=1)
            if product_exist.categ_id.id != 81:
                if product_exist.categ_id.id != 85:
                    product_name = ' '
                    component_qty  = 0.0
                    uom_id = ' '
                    for product in bom_product:                
        ...
        if uniq_list:        
            ...
            raise UserError("There is not any Record")

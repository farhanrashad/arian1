...
class de_limit_receiving_delivery(models.Model):
    ...
    def button_validate(self):
        ...
        # transfers.
        print("Entered in Button Validate")
                        ...
                        for rec_do in delivery_order_ids:
                            for rec_prod_do in rec_do.move_line_ids_without_package:
                                if rec_bom.product_id == rec_prod_do.product_id.id:
                                    if rec_bom.product_uom_qty <= rec_prod_do.qty_done:
                                        pass
                                    else:
                                        raise UserError("Product : "+str(rec_prod.name)+" cannot be received as its component has not been delivered yet")
                    else:
                        raise UserError("Product : " +str(rec_prod.name)+ " cannot be received as its component: has not been delivered yet")

    ...
    def action_create_bill(self):
        ...
        for agent in list:
            ...
            credit_line = (0, 0, {
                                'name': 'Commission Bill',
                                'debit': 0.0,
                                'credit': total_commission_amount ,
                                'partner_id': agent.id,
                                'account_id': agent.property_account_payable_id.id,
                              })
            line_ids.append(credit_line)
            credit_sum += credit_line[2]['credit'] - credit_line[2]['debit']

            move_dict['line_ids'] = line_ids
            move = self.env['account.invoice'].create(move_dict)   

...
class de_sale_order(models.Model):
    ...
    product_tmpl_ids = fields.One2many('sale.order.total', 'order_id', string="Total Qty Amount")
    is_compute_qty_amount = fields.Boolean()
    
    @api.depends('order_line.product_uom_qty', 'order_line.price_subtotal')
    def change_order_compute(self):
        for input in self.product_tmpl_ids:
            input.unlink()
        if self.is_compute_qty_amount == True:
            self.is_compute_qty_amount = False
        else:
            self.is_compute_qty_amount = True
        if self.is_compute_qty_amount == True:
            variants = []
            variant_name = ''
